...
public class ExecutionTimeDAO extends GenericsDAO<ExecutionTimeVO, Integer> {
	...
	public List<ExecutionTimeVO> listByActivity(int activityID) {
		Query q = entityManager.createQuery("select p FROM ExecutionTimeVO p where p.activityVO.id=:activity ",
				ExecutionTimeVO.class);
	...
	public int executionInProgressByUser(UserVO userVO) {
		Query q = entityManager.createQuery(
				"SELECT p FROM ExecutionTimeVO p WHERE p.finishDate = null and p.activityVO.userVO = :user",
				ExecutionTimeVO.class);
	...
	public ExecutionTimeVO executionInProgress(int activityID) {
		Query q = entityManager.createQuery(
				"SELECT p FROM ExecutionTimeVO p WHERE p.activityVO.id = :activity and p.finishDate = null",
				ExecutionTimeVO.class);
		q.setParameter("activity", activityID);
		try {
